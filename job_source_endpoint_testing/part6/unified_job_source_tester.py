#!/usr/bin/env python3
"""
Unified simple job-source tester for Part 6 companies.

Purpose:
  Test official source access methods intended for job_search_6.
...
def main() -> int:
    tests = [
        lambda: test_greenhouse("Torc Robotics", "torcrobotics"),
        lambda: test_ashby("Applied Intuition", "applied"),
        lambda: test_greenhouse("Aurora", "aurorainnovation"),
        lambda: test_ashby("Skydio", "skydio"),
        lambda: test_workday(
            "Boston Dynamics",
        ...
        lambda: test_greenhouse("Zipline", "flyzipline"),
        lambda: test_greenhouse("Figure AI", "figureai"),
        lambda: test_greenhouse("Agility Robotics", "agilityrobotics"),
        lambda: test_lever("Waabi", "waabi"),
    ]
