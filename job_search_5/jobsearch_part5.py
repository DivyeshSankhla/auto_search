#!/usr/bin/env python3
"""
Part 5 job-search automation.

This script implements the repeatable /jobsearch flow for job_search_5 only:
...
OFFICIAL_DOMAINS = {
    ...
    "Panasonic": ("careers.na.panasonic.com",),
    "Canonical": ("boards-api.greenhouse.io", "job-boards.greenhouse.io", "boards.greenhouse.io", "canonical.com"),
    "Roku": ("boards-api.greenhouse.io", "job-boards.greenhouse.io", "boards.greenhouse.io", "weareroku.com"),
    "Sonos": ("sonos.wd1.myworkdayjobs.com",),
    "Bose": ("boseallaboutme.wd503.myworkdayjobs.com",),
    "Verkada": ("boards-api.greenhouse.io", "job-boards.greenhouse.io", "boards.greenhouse.io", "verkada.com"),
    "Samsara": ("boards-api.greenhouse.io", "job-boards.greenhouse.io", "boards.greenhouse.io", "samsara.com"),
    "Zoox": ("jobs.lever.co", "zoox.com", "api.lever.co"),
    "Rivian / Rivian VW": ("careers.rivian.com", "rivianvw.tech", "api.ashbyhq.com", "jobs.ashbyhq.com"),
}
...
class LeverAdapter(BaseAdapter):
    ...
    def __init__(self, company: str, company_slug: str) -> None:
        self.company = company
        self.company_slug = company_slug

    def fetch(self, ctx: RunContext) -> AdapterResult:
        params = {"mode": "json", "limit": str(ctx.limit)}
        url = f"https://api.lever.co/v0/postings/{self.company_slug}?" + urllib.parse.urlencode(params)
        code, body = http_get(ctx, url)
        data = load_json_body(body)
        items = data if isinstance(data, list) else []
        items.sort(key=lambda job: int(job.get("createdAt") or 0) if isinstance(job, dict) else 0, reverse=True)

        us_items = [
            item for item in items
            if isinstance(item, dict)
            and (is_us_text(item.get("country")) or is_us_text((item.get("categories") or {}).get("location")))
        ]
        selected = us_items or [item for item in items if isinstance(item, dict)]
        ...
        for item in selected:
            ...
            desc = html_to_text(str(item.get("description") or item.get("descriptionPlain") or ""))
            ...
            raw = RawJob(
                company=self.company,
                source_method=self.method,
                title=first_text(item.get("text")),
                url=job_url,
                job_id=job_id,
                requisition_id=job_id,
                location=location or "United States",
                posted_raw=str(item.get("createdAt") or ""),
                posted_date=posted,
                description=" ".join([first_text(item.get("text")), desc]).strip(),
                raw=item,
            )
            jobs.append(raw)
            if len(jobs) >= ctx.limit:
...
class RivianCombinedAdapter(BaseAdapter):
    company = "Rivian / Rivian VW"
    method = "requests"
    ...
    def fetch(self, ctx: RunContext) -> AdapterResult:
        ...
        jibe_url = "https://careers.rivian.com/api/jobs?" + urllib.parse.urlencode(jibe_params)
        code, body = http_get(ctx, jibe_url)
        last_code = code
        if code == 200:
            data = load_json_body(body)
            batch = data.get("jobs", []) if isinstance(data, dict) else []
            ...
            for item in batch:
                if not isinstance(item, dict):
                    continue
                data_obj = item.get("data", item) if isinstance(item.get("data"), dict) else item
                job_url = canonical_from_jibe(item, "https://careers.rivian.com")
        ...
        ashby_url = "https://api.ashbyhq.com/posting-api/job-board/rivianvw.tech"
        vw_code, vw_body = http_get(ctx, ashby_url)
        if last_code != 200 and vw_code == 200:
            last_code = vw_code
        if vw_code == 200 and len(jobs) < ctx.limit:
            vw_data = load_json_body(vw_body)
            vw_items = vw_data.get("jobs", []) if isinstance(vw_data, dict) else []
            vw_items = vw_items if isinstance(vw_items, list) else []
...
def build_adapters() -> List[BaseAdapter]:
    return [
        ...
        JibeAdapter("Panasonic", "https://careers.na.panasonic.com", "https://careers.na.panasonic.com/api/jobs"),
        GreenhouseAdapter("Canonical", "canonical"),
        GreenhouseAdapter(
            "Roku",
            "roku",
            public_url_template="https://www.weareroku.com/jobs/{id}?gh_jid={id}",
        ),
        WorkdayCXSAdapter(
            "Sonos",
        ...
        GreenhouseAdapter("Verkada", "verkada"),
        GreenhouseAdapter(
            "Samsara",
            "samsara",
            public_url_template="https://www.samsara.com/company/careers/roles/{id}?gh_jid={id}",
        ),
        LeverAdapter("Zoox", "zoox"),
        RivianCombinedAdapter(),
    ]
...
def location_ok(raw: RawJob) -> bool:
    ...
    if not text:
        return raw.company in {"Panasonic", "Zoox", "Rivian / Rivian VW", "Canonical"}
...
def score_job(raw: RawJob, profile: Dict[str, Any], ctx: RunContext) -> Tuple[int, str]:
    ...
    if re.search(r"\b(principal|staff|lead|architect)\b", title):
        score -= 4
    if raw.company in {"Canonical", "Roku", "Verkada", "Samsara", "Zoox", "Rivian / Rivian VW"}:
        score += 3
...
def run_search(args: argparse.Namespace) -> int:
    ...
    print(f"Part 5 search: limit={ctx.limit}, days={ctx.days}, cutoff={ctx.cutoff.isoformat()}")
...
def run_self_tests() -> int:
    ...
    loc_raw = RawJob("Sonos", "requests", "t", "https://x", location="US, California, Santa Barbara")
    check("location us state city", location_ok(loc_raw))

    check("official url roku", is_official_url("Roku", "https://www.weareroku.com/jobs/99?gh_jid=99"))
    ...
    good = RawJob(
        ...
        url="https://www.verkada.com/careers/openings/engineering/12345",
        job_id="12345",
    ...
    existing = {"https://www.verkada.com/careers/openings/engineering/12345"}
    dup = dataclasses.replace(good, url="https://www.verkada.com/careers/openings/engineering/12345")
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Part 5 official-source job search.")
