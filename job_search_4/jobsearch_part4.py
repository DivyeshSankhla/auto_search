#!/usr/bin/env python3
"""
Part 4 job-search automation.

This script implements the repeatable /jobsearch flow for job_search_4 only:
...
OFFICIAL_DOMAINS = {
    ...
    "Intuitive Surgical": ("intuitive.wd1.myworkdayjobs.com",),
    "GE HealthCare": ("gehc.wd5.myworkdayjobs.com",),
    "Siemens Healthineers": ("jobs.siemens-healthineers.com",),
    "Dexcom": ("dexcom.wd1.myworkdayjobs.com",),
    "Rockwell Automation": ("rockwellautomation.wd1.myworkdayjobs.com",),
    "Schneider Electric": ("careers.se.com",),
    "Eaton": ("eaton.eightfold.ai",),
    "Caterpillar": ("careers.caterpillar.com",),
    "John Deere": ("jobs.deere.com",),
}
...
def parse_searchjobs_cards(body: str, host: str) -> List[Tuple[str, str, str]]:
    ...
    link_re = re.compile(
        r'href=["\'](?P<href>https://%s/[^"\']*/JobDetail/(?P<id>\d+))["\']' % re.escape(host),
        flags=re.I,
    )
    for article in article_pattern.findall(body):
        link_match = link_re.search(article)
        title_match = re.search(
            r'<a[^>]*class=["\']link["\'][^>]*href=["\'][^"\']*/JobDetail/\d+["\'][^>]*>(?P<title>.*?)</a>',
            article,
            flags=re.I | re.S,
        )
...
def parse_static_job_links(body: str, base: str, href_pattern: str) -> List[Tuple[str, str]]:
    ...
    for match in re.finditer(
        r'href=["\'](?P<href>%s)["\'][^>]*>(?P<title>.*?)</a>' % href_pattern,
        body,
        flags=re.I | re.S,
    ):
...
class SearchJobsAdapter(BaseAdapter):
    method = "requests"

    def __init__(self, company: str, base_url: str, host: str) -> None:
        self.company = company
    ...
    def fetch(self, ctx: RunContext) -> AdapterResult:
        ...
        for offset in range(0, ctx.limit, 6):
            ...
            if code != 200:
                break
            for title, link, location in parse_searchjobs_cards(body, self.host):
                if link in seen:
                    continue
                seen.add(link)
...
class EightfoldPCSAdapter(BaseAdapter):
    ...
    def __init__(self, company: str, base: str, domain: str) -> None:
        self.company = company
        self.base = base.rstrip("/")
        self.domain = domain
    ...
    def fetch(self, ctx: RunContext) -> AdapterResult:
        ...
        if code == 200:
            ...
            for pos in positions:
                ...
                raw_url = str(pos.get("positionUrl") or "")
...
class CaterpillarXMLAdapter(BaseAdapter):
    company = "Caterpillar"
    method = "requests"
    ...
    def fetch(self, ctx: RunContext) -> AdapterResult:
        ...
        for block in re.findall(r"<job>[\s\S]*?</job>", body):
            ...
            date_match = re.search(r"<date><!\[CDATA\[(?P<date>[\s\S]*?)\]\]></date>", block)
            req_match = re.search(r"<requisitionid><!\[CDATA\[(?P<req>[\s\S]*?)\]\]></requisitionid>", block)
            ...
            job_url = clean_text(url_match.group("url"))
            if job_url in seen:
                continue
            seen.add(job_url)

            req_id = clean_text(req_match.group("req")) if req_match else ""
            posted_raw = clean_text(date_match.group("date")) if date_match else ""
            posted = parse_date(posted_raw)
...
class JohnDeereAdapter(BaseAdapter):
    company = "John Deere"
    method = "requests"
    ...
    def fetch(self, ctx: RunContext) -> AdapterResult:
        ...
        for startrow in range(0, ctx.limit, 25):
            params = {
                "q": "",
            ...
            if code != 200:
                break
            for title, link in parse_static_job_links(
                body, "https://jobs.deere.com", r"/(?:eightfold|successfactors)/job/[^\"']+"
            ):
                if link in seen:
                    continue
                seen.add(link)
...
def build_adapters() -> List[BaseAdapter]:
    return [
        ...
        JibeAdapter("Schneider Electric", "https://careers.se.com", "https://careers.se.com/api/jobs"),
        EightfoldPCSAdapter("Eaton", "https://eaton.eightfold.ai", "eaton.com"),
        CaterpillarXMLAdapter(),
        JohnDeereAdapter(),
    ]
...
def location_ok(raw: RawJob) -> bool:
    ...
    if not text:
        return raw.company in {"Siemens Healthineers", "Caterpillar", "John Deere", "Eaton"}
...
def score_job(raw: RawJob, profile: Dict[str, Any], ctx: RunContext) -> Tuple[int, str]:
    ...
    if re.search(r"\b(principal|staff|lead|architect)\b", title):
        score -= 4
    if raw.company in {"Medtronic", "Dexcom", "Rockwell Automation", "Schneider Electric", "John Deere"}:
        score += 3
...
def run_search(args: argparse.Namespace) -> int:
    ...
    print(f"Part 4 search: limit={ctx.limit}, days={ctx.days}, cutoff={ctx.cutoff.isoformat()}")
...
def run_self_tests() -> int:
    ...
    healthineers_html = (
        '<article><a class="link" href="https://jobs.siemens-healthineers.com/en_US/searchjobs/JobDetail/505301">'
        "Embedded Engineer</a><span class=\"list-item-location\">Princeton, NJ</span></article>"
    )
    health_cards = parse_searchjobs_cards(healthineers_html, "jobs.siemens-healthineers.com")
    check("searchjobs parser", len(health_cards) == 1 and "505301" in health_cards[0][1])

    cat_xml = (
        "<job><title><![CDATA[Firmware Engineer]]></title>"
        "<date><![CDATA[2026-06-18]]></date>"
        "<requisitionid><![CDATA[R123]]></requisitionid>"
        "<url><![CDATA[https://careers.caterpillar.com/en/jobs/r123]]></url>"
        "<country><![CDATA[United States]]></country></job>"
    )
    check("caterpillar us country", is_us_text("United States"))
    check("caterpillar xml title", "Firmware Engineer" in cat_xml)

    deere_html = (
        '<a href="/eightfold/job/Fargo-Senior-Power-Electronics-Control-System-Engineer-North">'
        "Senior Power Electronics Control System Engineer</a>"
    )
    deere_links = parse_static_job_links(deere_html, "https://jobs.deere.com", r"/(?:eightfold|successfactors)/job/[^\"']+")
    check("john deere parser", len(deere_links) == 1 and "eightfold" in deere_links[0][1])
    ...
    check("dexcom state facet", is_us_text("California"))

    loc_raw = RawJob("Dexcom", "requests", "t", "https://x", location="US, California, San Diego")
    check("location us state city", location_ok(loc_raw))

    check("official url medtronic", is_official_url("Medtronic", "https://medtronic.wd1.myworkdayjobs.com/MedtronicCareers/job/1"))
    ...
    good = RawJob(
        ...
        url="https://rockwellautomation.wd1.myworkdayjobs.com/External_Rockwell_Automation/job/Milwaukee/Embedded-Firmware_R123",
        job_id="R123",
    ...
    existing = {"https://rockwellautomation.wd1.myworkdayjobs.com/External_Rockwell_Automation/job/Milwaukee/Embedded-Firmware_R123"}
    dup = dataclasses.replace(good, url="https://rockwellautomation.wd1.myworkdayjobs.com/External_Rockwell_Automation/job/Milwaukee/Embedded-Firmware_R123")
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Part 4 official-source job search.")
