#!/usr/bin/env python3
"""
Part 8 job-search automation.

This script implements the repeatable /jobsearch flow for job_search_8 only:
...
COMPANY_TERMS = {
    "Cognex": ["machine vision", "embedded", "camera", "industrial", "inspection"],
    "Axis Communications": ["video surveillance", "embedded", "camera", "network", "iot"],
    "Arlo": ["smart home", "camera", "embedded", "iot", "security"],
    "Ubiquiti": ["networking", "wifi", "embedded", "firmware", "unifi"],
    "Netgear": ["networking", "wifi", "embedded", "router", "firmware"],
    "TP-Link": ["networking", "wifi", "embedded", "router", "firmware"],
    "Ambarella": ["computer vision", "soc", "embedded", "camera", "ai"],
    "ON Semiconductor": ["semiconductor", "embedded", "power", "sensor", "analog"],
    "MediaTek": ["semiconductor", "soc", "embedded", "wireless", "connectivity"],
    "Renesas": ["semiconductor", "embedded", "mcu", "automotive", "firmware"],
}

US_LOCATION_TEXT = ("united states", "united states of america", "usa", "us,", "us ")

OFFICIAL_DOMAINS = {
    "Cognex": ("cognex.wd1.myworkdayjobs.com", "cognex.com"),
    "Axis Communications": ("axis.wd3.myworkdayjobs.com", "axis.com"),
    "Arlo": ("arlo.wd12.myworkdayjobs.com", "arlo.com"),
    "Ubiquiti": ("boards-api.greenhouse.io", "job-boards.greenhouse.io", "ui.com"),
    "Netgear": ("api.ashbyhq.com", "jobs.ashbyhq.com", "netgear.com"),
    "TP-Link": ("apply.workable.com", "tp-link.com"),
    "Ambarella": ("ambarella.wd108.myworkdayjobs.com", "ambarella.com"),
    "ON Semiconductor": ("hctz.fa.us2.oraclecloud.com", "onsemi.com"),
    "MediaTek": ("careers.mediatek.com", "mediatek.com"),
    "Renesas": ("jobs.smartrecruiters.com", "api.smartrecruiters.com", "renesas.com"),
}
...
class WorkableAdapter(BaseAdapter):
    ...
    def __init__(self, company: str, board_slug: str) -> None:
        self.company = company
        self.board_slug = board_slug

    def fetch(self, ctx: RunContext) -> AdapterResult:
        query = urllib.parse.urlencode({"location[0][country]": "United States"})
        url = f"https://apply.workable.com/{self.board_slug}/jobs.md?{query}"
        code, body = http_get(ctx, url, accept="text/markdown,text/plain,*/*")
        items = parse_workable_markdown(body, self.board_slug)
        us_items = [item for item in items if is_us_text(item.get("location"))]
        selected = us_items or items

        jobs: List[RawJob] = []
        seen: set[str] = set()
        for item in selected:
            key = str(item.get("shortcode") or item.get("url") or "")
            if not key or key in seen:
                continue
            seen.add(key)
...
class MediaTekCrawlAdapter(BaseAdapter):
    company = "MediaTek"
    method = "requests"
    ...
    def fetch(self, ctx: RunContext) -> AdapterResult:
        ...
        while queue and len(jobs) < ctx.limit:
            job_id = queue.pop(0)
            if job_id in seen:
                continue
            seen.add(job_id)
            code, body, title, related = mediatek_fetch_detail(opener, job_id)
            last_code = code
            if code == 200 and title:
                ...
                if body:
                    raw.description = " ".join([title, html_to_text(body)]).strip()[:8000]
                jobs.append(raw)
    ...
    def fetch(self, ctx: RunContext) -> AdapterResult:
        ...
        for job in selected:
            ...
            flex_text = " ".join(
                ...
                for f in flex if isinstance(f, dict)
            )
            ...
            raw = RawJob(
                ...
                url=f"{self.public_base}/job/{req_id}",
                job_id=req_id,
                requisition_id=req_id,
                location=location,
                posted_raw=posted_raw,
...
def build_adapters() -> List[BaseAdapter]:
    return [
        ...
        GreenhouseAdapter("Ubiquiti", "ubiquiti"),
        AshbyAdapter("Netgear", "netgear"),
        WorkableAdapter("TP-Link", "tp-link-usa-corp"),
        WorkdayCXSAdapter(
            "Ambarella",
        ...
        MediaTekCrawlAdapter(),
        SmartRecruitersAdapter("Renesas", "renesaselectronics", "RenesasElectronics"),
    ]
...
def run_search(args: argparse.Namespace) -> int:
    ...
    print(f"Part 8 search: limit={ctx.limit}, days={ctx.days}, cutoff={ctx.cutoff.isoformat()}")
...
def run_self_tests() -> int:
    ...
    loc_raw = RawJob("Cognex", "requests", "t", "https://x", location="US, Massachusetts, Natick")
    check("location us state city", location_ok(loc_raw))

    check("official url serve", is_official_url("Netgear", "https://jobs.ashbyhq.com/netgear/abc"))
    ...
    good = RawJob(
        ...
        url="https://cognex.wd1.myworkdayjobs.com/External_Career_Site/job/Natick/Embedded-Firmware_R123",
        job_id="R123",
    ...
    existing = {"https://cognex.wd1.myworkdayjobs.com/External_Career_Site/job/Natick/Embedded-Firmware_R123"}
    dup = dataclasses.replace(good, url="https://cognex.wd1.myworkdayjobs.com/External_Career_Site/job/Natick/Embedded-Firmware_R123")
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Part 8 official-source job search.")
