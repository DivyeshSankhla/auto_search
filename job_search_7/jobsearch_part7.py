#!/usr/bin/env python3
"""
Part 7 job-search automation.

This script implements the repeatable /jobsearch flow for job_search_7 only:
...
OFFICIAL_DOMAINS = {
    "Nuro": ("boards-api.greenhouse.io", "job-boards.greenhouse.io", "boards.greenhouse.io", "nuro.ai"),
    "Kodiak Robotics": ("boards-api.greenhouse.io", "job-boards.greenhouse.io", "boards.greenhouse.io", "kodiak.ai"),
    "Gatik": ("boards-api.greenhouse.io", "job-boards.greenhouse.io", "boards.greenhouse.io", "gatik.ai"),
    "May Mobility": ("boards-api.greenhouse.io", "job-boards.greenhouse.io", "boards.greenhouse.io", "maymobility.com"),
    "Avride": ("boards-api.greenhouse.io", "job-boards.greenhouse.io", "boards.greenhouse.io", "avride.ai"),
    "Serve Robotics": ("api.ashbyhq.com", "jobs.ashbyhq.com", "serverobotics.com"),
    "Gecko Robotics": ("api.ashbyhq.com", "jobs.ashbyhq.com", "geckorobotics.com"),
    "Skild AI": ("boards-api.greenhouse.io", "job-boards.greenhouse.io", "boards.greenhouse.io", "skild.ai"),
    "Motorola Solutions / Avigilon": ("motorolasolutions.wd5.myworkdayjobs.com", "motorolasolutions.com", "avigilon.com"),
    "Zebra Technologies": ("zebra.wd501.myworkdayjobs.com", "zebra.com"),
}
...
class MayMobilityGreenhouseAdapter(BaseAdapter):
    method = "requests"
    company = "May Mobility"

    def fetch(self, ctx: RunContext) -> AdapterResult:
        primary = GreenhouseAdapter("May Mobility", "maymobility")
        result = primary.fetch(ctx)
        if result.raw_jobs:
            return result
        fallback = GreenhouseAdapter("May Mobility", "maymobilityjobs")
        return fallback.fetch(ctx)


def build_adapters() -> List[BaseAdapter]:
    return [
        GreenhouseAdapter("Nuro", "nuro"),
        GreenhouseAdapter("Kodiak Robotics", "kodiak"),
        GreenhouseAdapter("Gatik", "gatikaiinc"),
        MayMobilityGreenhouseAdapter(),
        GreenhouseAdapter("Avride", "avride"),
        AshbyAdapter("Serve Robotics", "serverobotics"),
        AshbyAdapter("Gecko Robotics", "gecko-robotics"),
        GreenhouseAdapter("Skild AI", "skildai-careers"),
        WorkdayCXSAdapter(
            "Motorola Solutions / Avigilon",
...
def run_search(args: argparse.Namespace) -> int:
    ...
    print(f"Part 7 search: limit={ctx.limit}, days={ctx.days}, cutoff={ctx.cutoff.isoformat()}")
...
def run_self_tests() -> int:
    ...
    loc_raw = RawJob("Motorola Solutions / Avigilon", "requests", "t", "https://x", location="US, Illinois, Chicago")
    check("location us state city", location_ok(loc_raw))

    check("official url serve", is_official_url("Serve Robotics", "https://jobs.ashbyhq.com/serverobotics/abc"))
    ...
    good = RawJob(
        ...
        url="https://motorolasolutions.wd5.myworkdayjobs.com/Careers/job/Chicago/Embedded-Firmware_R123",
        job_id="R123",
    ...
    existing = {"https://motorolasolutions.wd5.myworkdayjobs.com/Careers/job/Chicago/Embedded-Firmware_R123"}
    dup = dataclasses.replace(good, url="https://motorolasolutions.wd5.myworkdayjobs.com/Careers/job/Chicago/Embedded-Firmware_R123")
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Part 7 official-source job search.")
