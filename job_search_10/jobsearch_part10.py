#!/usr/bin/env python3
"""
Part 10 job-search automation.

This script implements the repeatable /jobsearch flow for job_search_10 only:
...
COMPANY_TERMS = {
    "Wabtec": ["rail", "transportation", "embedded", "industrial", "automation"],
    "Resideo": ["smart home", "hvac", "iot", "embedded", "security"],
    "Brivo": ["access control", "security", "cloud", "embedded", "iot"],
    "Wyze": ["smart home", "camera", "iot", "embedded", "firmware"],
    "GoPro": ["camera", "action camera", "embedded", "firmware", "video"],
}

US_LOCATION_TEXT = ("united states", "united states of america", "usa", "us,", "us ")

OFFICIAL_DOMAINS = {
    "Wabtec": ("jobs.smartrecruiters.com", "api.smartrecruiters.com", "wabtec.com"),
    "Resideo": ("ehtl.fa.us6.oraclecloud.com", "resideo.com"),
    "Brivo": ("careers.brivo.com", "brivo.com"),
    "Wyze": ("jobs.gem.com", "wyze.com"),
    "GoPro": ("boards-api.greenhouse.io", "job-boards.greenhouse.io", "boards.greenhouse.io", "gopro.com"),
}
...
class PinpointAdapter(BaseAdapter):
    ...
    def __init__(self, company: str, postings_url: str) -> None:
        self.company = company
        self.postings_url = postings_url

    def fetch(self, ctx: RunContext) -> AdapterResult:
        code, body = http_get(ctx, self.postings_url, accept="application/json,*/*")
        data = load_json_body(body)
        ...
        us_items = [item for item in items if isinstance(item, dict) and is_us_text(pinpoint_location_text(item))]
        selected = us_items or [i for i in items if isinstance(i, dict)]
        ...
        for item in selected:
            ...
            raw = RawJob(
                ...
                job_id=str(item.get("id") or key),
                location=pinpoint_location_text(item),
                posted_raw=str(item.get("published_at") or item.get("created_at") or ""),
                posted_date=posted,
                description=first_text(item.get("title"), item.get("description")),
                raw=item,
            )
            enrich_from_html_page(ctx, raw)
            jobs.append(raw)
...
class GemGraphQLAdapter(BaseAdapter):
    method = "requests"

    def __init__(self, company: str, board_id: str) -> None:
        self.company = company
    ...
    def fetch(self, ctx: RunContext) -> AdapterResult:
        ...
        code, body = http_post_json(ctx, "https://jobs.gem.com/api/public/graphql", payload)
        data = load_json_body(body)
        items: List[Dict[str, Any]] = []
        if isinstance(data, dict):
        ...
        us_items = [item for item in items if isinstance(item, dict) and is_us_text(gem_location_text(item))]
        selected = us_items or [i for i in items if isinstance(i, dict)]

        jobs: List[RawJob] = []
        seen: set[str] = set()
        for item in selected:
            ext_id = str(item.get("extId") or "")
            key = ext_id or str(item.get("title") or "")
            if not key or key in seen:
                continue
            seen.add(key)
            url = f"https://jobs.gem.com/{self.board_id}/{ext_id}" if ext_id else f"https://jobs.gem.com/{self.board_id}"
            raw = RawJob(
                company=self.company,
                source_method=self.method,
                title=first_text(item.get("title")),
                url=url,
                job_id=ext_id or key,
                location=gem_location_text(item),
                description=first_text(item.get("title")),
                raw=item,
            )
            enrich_from_html_page(ctx, raw)
            jobs.append(raw)
...
def build_adapters() -> List[BaseAdapter]:
    return [
        SmartRecruitersAdapter("Wabtec", "Wabtec", "Wabtec"),
        OracleCEAdapter(
            "Resideo",
        ...
        PinpointAdapter("Brivo", "https://careers.brivo.com/postings.json"),
        GemGraphQLAdapter("Wyze", "wyzecam-com"),
        GreenhouseAdapter("GoPro", "goprocareers"),
    ]
...
def run_search(args: argparse.Namespace) -> int:
    ...
    print(f"Part 10 search: limit={ctx.limit}, days={ctx.days}, cutoff={ctx.cutoff.isoformat()}")
...
def run_self_tests() -> int:
    ...
    loc_raw = RawJob("Cognex", "requests", "t", "https://x", location="US, Massachusetts, San Mateo")
    check("location us state city", location_ok(loc_raw))

    check("official url serve", is_official_url("GoPro", "https://boards.greenhouse.io/goprocareers/jobs/123"))
    ...
    good = RawJob(
        ...
        url="https://boards.greenhouse.io/goprocareers/job/San Mateo/Embedded-Firmware_R123",
        job_id="R123",
    ...
    existing = {"https://boards.greenhouse.io/goprocareers/job/San Mateo/Embedded-Firmware_R123"}
    dup = dataclasses.replace(good, url="https://boards.greenhouse.io/goprocareers/job/San Mateo/Embedded-Firmware_R123")
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Part 10 official-source job search.")
